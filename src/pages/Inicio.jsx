...
export default function Inicio({ playerConfig, searchOpen, setSearchOpen, onWatchingChange }) {
  ...
  const [catTab,       setCatTab]       = useState('noticias') // 'noticias' | 'videos' dentro de una categoría
  ...
  // Cargar noticias (posts de "Noticias y más") desde Firestore
  useEffect(() => {
  ...
  const getSeasonOf = (ep) => {
    ...
    const num = parseNum(ep.episode) ?? 0
  ...
  // Las temporadas NO están limitadas a SEASON_RANGES: si un episodio
  // tiene una temporada que no existía antes (ej. "8"), esa temporada
  // se crea automáticamente aquí con solo tener un episodio asignado.
  const episodesBySeason = allSorted
    .filter(ep => !ep.category || ep.category === '') // solo La Rosa de Guadalupe
    .reduce((acc, ep) => {
      const s = getSeasonOf(ep)
      let group = acc.find(g => g.season === s)
      if (!group) { group = { season: s, episodes: [] }; acc.push(group) }
  ...
  if (activeCategory !== null) {
    ...
    const catNews = newsList.filter(n => n.category === activeCategory.id)
      ...
      <div className={styles.detailWrapper}>
        ...
        <div className={styles.mobileDetailOnly}>
          ...
          <div className={styles.catTabs}>
            <button
              className={`${styles.catTab} ${catTab === 'noticias' ? styles.catTabActive : ''}`}
              onClick={() => setCatTab('noticias')}
            >
              Noticias y más
            </button>
            <button
              className={`${styles.catTab} ${catTab === 'videos' ? styles.catTabActive : ''}`}
              onClick={() => setCatTab('videos')}
            >
              Videos
            </button>
          </div>
          ...
          {catTab === 'videos' && (
            <div className={styles.catEpisodeList}>
              {catEpisodes.length === 0
                ? <p style={{ color: '#888', padding: '20px', textAlign: 'center', fontSize: 13 }}>No hay episodios en esta categoría aún.</p>
                : catEpisodes.map(ep => (
                  <div key={ep.id} className={styles.catEpisodeRow} onClick={() => openEpisode(ep)}>
                    <div className={styles.catEpisodeThumbWrap}>
                      <img src={ep.thumbnail || FALLBACK_THUMB} alt={ep.title} className={styles.catEpisodeThumb} />
                      <div className={styles.catEpisodePlayCircle}><Play size={18} fill="#fff" color="#fff" style={{ marginLeft: 2 }} /></div>
                      {ep.duration && (
                        <span className={styles.catEpisodeDurBadge}>
                          <Play size={9} fill="#fff" color="#fff" />
                          {ep.duration}
                        </span>
                      )}
                    </div>
                    <div className={styles.catEpisodeInfo}>
                      <h3 className={styles.catEpisodeTitle}>{ep.title}</h3>
                      {ep.description && <p className={styles.catEpisodeDesc}>{ep.description}</p>}
                    </div>
                  </div>
                ))
              }
          ...
          {catTab === 'noticias' && (
            <div className={styles.newsList}>
              {catNews.length === 0
                ? <p style={{ color: '#888', padding: '20px', textAlign: 'center', fontSize: 13 }}>No hay noticias publicadas todavía.</p>
                : catNews.map((n, i) => {
                  const isExpanded = expandedNews === n.id
                  return (
                    <div key={n.id} className={styles.newsCard} onClick={() => setExpandedNews(isExpanded ? null : n.id)}>
                      {n.image && <img src={n.image} alt={n.title} className={styles.newsImage} />}
                      <div className={styles.newsBody}>
                        {i === 0 && <span className={styles.newsBadge}>NUEVO</span>}
                        <h3 className={styles.newsTitle}>{n.title}</h3>
                        {n.text && (
                          <p className={`${styles.newsText} ${isExpanded ? styles.newsTextExpanded : ''}`}>{n.text}</p>
                        )}
                        {getPubDate(n) && <span className={styles.newsDate}>{getPubDate(n)}</span>}
                      </div>
                    </div>
                  )
                })
        ...
        <div className={styles.desktopDetailOnly}>
          ...
          {catTab === 'noticias' && (
            <div className={styles.newsList}>
              {catNews.length === 0
                ? <div className={styles.detailEmpty}><p>No hay noticias publicadas todavía.</p></div>
                : catNews.map((n, i) => {
                  const isExpanded = expandedNews === n.id
                  return (
                    <div key={n.id} className={styles.newsCard} onClick={() => setExpandedNews(isExpanded ? null : n.id)}>
        ...
        <>
          ...
          <section className={styles.catSection}>
            ...
            <div className={styles.catSlider}>
              ...
              {extraCats.map((cat, i) => (
                <FadeInCard key={cat.id} className={styles.catCard} delay={(i + 1) * 60} onClick={() => { setActiveCategory(cat); setCatTab('noticias'); setExpandedNews(null) }}>
                  <div className={styles.catImgWrap}>
                    {cat.thumbnail
